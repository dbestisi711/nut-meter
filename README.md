...
- The frontend calls `GET /api/nuts` to load the shared data.
- For mutations, it POSTs an action (`addNuts`, `resetDay`, `createUser`, `removePlayer`) and the server applies it to the current state. This avoids race conditions when two browsers are active at once.
